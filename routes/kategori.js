...
router.get('/', async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM kategori ORDER BY id_kategori ASC');
...
router.post('/', async (req, res) => {
  ...
  try {
    await db.query('INSERT INTO kategori (kode_kategori, nama_kategori) VALUES ($1, $2)', [kodeKategori, namaKategori]);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Kategori berhasil diupdate', kategori: updateResult.rows[0] });
  } catch (err) {
    console.error('Gagal update kategori:', err);
    res.status(500).json({ error: 'Terjadi kesalahan server saat update kategori' });
  }
});
